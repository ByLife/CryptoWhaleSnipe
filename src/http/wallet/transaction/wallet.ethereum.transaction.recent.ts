...
export default {
  ...
  run: async (req: express.Request, res: express.Response) => {
    try {
      ...
      if (!(await AccessBearer.findOne({ token: req.token })))
      ...
      for (const tx of transactions) {
        ...
        const walletAddress = wallet.wallets.includes(fromAddress)
          ? fromAddress
          : toAddress;
        ...
        if (tx.type === "swap") {
          ...
          secondaryTokenSymbol =
            tx.finalToken?.symbol ||
            tx.inTokens?.[tx.inTokens.length - 1]?.symbol ||
            "";
        ...
        results.push({
          ...
          timeStamp: tx.timestamp
            ? Math.floor(tx.timestamp.getTime() / 1000).toString()
            : "",
          usdPrice: usdPrice,
          value: value,
          type: tx.type,
